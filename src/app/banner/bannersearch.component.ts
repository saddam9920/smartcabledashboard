    ...
    activeUser(activevalue) {
        if (activevalue == '1') {
            if (this.activedeactivevalue == 'active') {
                this.userservice.activeUser({ "status": '1', "userid": this.activekey }).subscribe(
                    data => {
                        this.toastyService.success(' Successfully updated !');
                    },
                    err => {
                ...
                this.userservice.activeUser({ "status": '0', "userid": this.activekey }).subscribe(
                    data => {
                        this.toastyService.success(' Successfully updated !');
                    },
                    err => {
        ...
        this.userservice.getBannerById(id).subscribe(
            data => {
                ...
                this.optionSelected=this.result[0].BANNER_POSITION;
                                alert(this.result[0].BANNER_START_DATE)

                alert(this.result[0].START_DATE)
                this.userform.controls['StartDate'].setValue({date: {
            year: this.result[0].BANNER_START_DATE.split("-")[0],
            month: this.result[0].BANNER_START_DATE.split("-")[1],
            day:this.result[0].BANNER_START_DATE.split("-")[2]
        }});
                this.userform.controls['EndDate'].setValue(this.result[0].BANNER_END_DATE);
